#!/usr/bin/env bash
# tests/compiler_docs_smoke.sh
#
# Smoke coverage for the compiler-as-documentation thread: help/about text,
# workflow guidance, diagnostic codes, constraint warnings, explain lookup, and
...
search=$("$GRAFT_BIN" patch search --constraint TestsPass 2>&1 >/dev/null || true)
if ! grep -q 'constraint `TestsPass` is not declared' <<<"$search"; then
  echo "FAIL: search unknown constraint warning missing"
  echo "$search"; exit 1
fi
...
echo "OK: compiler-as-docs smoke covers help, workflow guidance, V003, Hole Report, warnings, and explain."
