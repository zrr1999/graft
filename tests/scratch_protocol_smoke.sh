#!/usr/bin/env bash
# tests/scratch_protocol_smoke.sh
#
# Smoke coverage for the scratch protocol seam:
...
create=$("$GRAFT" patch from-scratch "$seed_scratch" --message scratch-protocol-base)
...
candidate_out=$("$GRAFT" patch from-scratch "$scratch_after_delete" --message scratch-protocol-final)
