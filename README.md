...
- materialize 不写 cwd：`graft materialize <state-ref>` 先把 `tree:*`、`candidate:*`、`patch:*` 或 `repo:<id>@<treeish>` 解析成确定 state，再输出到 workspace `.worktrees/<state>/` 临时检查目录；`graft run <state-ref> -- <cmd>` 在临时 state root 执行命令并丢弃写入。
