...
//! properties. A workspace property binds a query, one evaluator, and a judge.
//!
...
impl Explainable for BuiltinEvaluatorMetadata {
    ...
    fn see_also(&self) -> &'static [&'static str] {
        match self.id {
            CHANGED_PATHS_ANY_MATCH => &["validate", "properties", "evidence-result.passed"],
            CHANGED_PATHS_ALL_MATCH => &["validate", "properties", "evidence-result.passed"],
