...
| 层       | 例子                                                                                                             | sync？                     | 丢了能恢复？              |
| ------- | -------------------------------------------------------------------------------------------------------------- | ------------------------- | ------------------- |
| public  | tree, action, application, change, constraint, plan, patch, evidence_refs(patch), relation, promotion, manifest, blob | 可选（由 workspace [sync] 决定） | 若已 sync 可从 remote 拉 |
...
Constraint(app)        : Type     # proof obligation over one concrete Application
Evidence             : Type     # runtime-generated witness of Constraint(app)
Admission(app)       : Type     # local decision that required constraints are satisfied
...
形式定义见 [`formal/kernel.lean`](../../formal/kernel.lean)：`State` / `Action` / `sem` /
...
旧 evidence 不能默认复用到新 Application，因为 `Constraint(Application)` 的 subject
绑定具体 base/action/proof/target。未来如果出现 action-level constraint，也应建模成
...
Evidence              : runtime-generated inhabitant of Constraint(Application)
```

`EvidenceRecord` 不是 constraint source 能自行构造的数学证明，
而是 Graft runtime 生成的可重建 proof-carrying data：给定
`(Application, plan, verifier, execution_contract)`，本地能在 sandboxed validation
中重跑并得到同一 `EvidenceId`，就等价于重新构造了该 constraint primitive 的 witness。
...
- `Sequence` flatten nested sequence；空 sequence 保留为 `Sequence([])`，是否拒绝 empty change 是 constraint/admission policy，不是 Action parser 的默认 gate。
...
形式定义见 [`formal/kernel.lean`](../../formal/kernel.lean) 的「核心模型/核心定义」段：
