...
mutable durable local bookkeeping（alias、remote 同步进度、查询索引）不在 `store/`，在 workspace `local/`；cwd route / repo local paths 在 `$GRAFT_HOME/registry.toml`，详见 §3.2 和 §12。目录名刻意不用 `state/`，以免与 patch 论里的 `State` / `StateId` 混淆。
